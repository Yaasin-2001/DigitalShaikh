...
const TemplateVarPanel: FC<ITemplateVarPanelProps> = ({
  ...
  return (
    <div className={cn(isFold ? 'border border-brown-500' : s.boxShodow, className, 'rounded-xl ')}>
      {/* header */}
      <div
        className={cn(isFold && 'rounded-b-xl', 'rounded-t-xl bg-white')}
      >
        {header}
      </div>
      {/* body */}
...
export const PanelTitle: FC<{ title: string; className?: string }> = ({
  ...
  return (
    <div className={cn(className, 'flex items-center space-x-1 text-brown-500')}>
      <StarIcon />
